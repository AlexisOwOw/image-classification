...
class CustomDTC():
    ...
    def build_tree(self, features, labels, depth):
        ...
        best_feature, best_threshold, _ = self.find_best_split(features, labels)
        X_left, X_right, y_left, y_right = self.split_data(features, labels, best_feature, best_threshold)
    ...
    def find_best_split(self, features, labels):
        ...
        for feature_index in range(n_features):
            ...
            for threshold in thresholds:
                x_left, x_right, y_left, y_right = self.split_data(features, labels, feature_index, threshold)
                if len(y_left) == 0 or len(y_right) == 0:  
                    continue
                gain = self.compute_gini_gain(labels, y_left, y_right)
                if gain > best_gain:
                    best_gain = gain
    ...
    def compute_gini_gain(self, parent_labels, left_labels, right_labels):
        parent_gini = self.compute_gini_impurity(parent_labels)
        n = len(parent_labels)
        n_left = len(left_labels)
        n_right = len(right_labels)

        weighted_gini = (n_left / n) * self.compute_gini_impurity(left_labels) + \
                    (n_right / n) * self.compute_gini_impurity(right_labels)
